...
function newslist($newsType, $language, $IsCommend, $newsLine,$pages=0) {
	global $maindb;
	$newsArray=array();
...
function productlist($productName, $typeId, $language, $IsCommend, $num, $pageD=1) {
	global $maindb;
	global $config;
	$proArray=array();
	$linkurl = $_SERVER ["PHP_SELF"] . "?";
...
if($PrevOrNext==0){
$ID="id<? ORDER BY OrderBy desc";
}else{
$ID="id>? ORDER BY OrderBy ";
}

 if (isset($nid) && $nid>0){
 $newsRs1 = $maindb->Execute("SELECT * FROM t_news WHERE  $IsShow  NewType=? AND $ID  LIMIT 0,1",array($NewType,$nid));
...
<?php
 ...
 //$pid 产品当前ID, $Called 产品所属分类ID ,$PrevOrNext 上一个还是下一个其中：0表示上一个 1表示下一个,$IsShow 是不是要显示推荐的新闻, $Language 表示新闻分类的语言。
 
function ProPrevNext($pid,$Called,$PrevOrNext,$Language,$IsShow,$name){
 global $maindb;
 $prev_next="";
 $ProType="";

if($Called!=""){
  $proRs = $maindb->Execute("SELECT ID FROM t_protype WHERE Language=? AND Called=?",array ($Language,$Called));
   if($proRs->RecordCount() == 1){
   $ProType = $proRs->fields[0];
   }
   }
...
$ID="id>? ORDER BY OrderBy";
 ...
 if (isset($pid) && $pid>0){
 $proRs1 = $maindb->Execute("SELECT * FROM t_products WHERE  $IsShow  TypeID=? AND $ID  LIMIT 0,1",array($ProType,$pid));
 if ($proRs1->RecordCount() >0){
		
		$proObj1 = $proRs1->FetchObject();

		$prev_next=$proObj1->ID;
		
	}
if($prev_next!=""){


	$prev_next="<a href='productsd.php?pid=".$proObj1->ID."'>".$name."</a>";


	}else{
	
	
  $prev_next="<a href='#'>".$name."</a>";
